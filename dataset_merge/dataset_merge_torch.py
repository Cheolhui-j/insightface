...
def get_embedding(args, imgrec, id, image_size, model, conf):
  ...
  while True:
    ...
    net_out = learner.model(batch.to(conf.device)).cpu()
    #random_num = random.randint(0, _batch_size) + ba -1
...
def main(args):
  ...
  if len(args.model)>0:
    ...
    # cvd = os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'
    # if len(cvd)>0:
  ...
  if len(args.exclude)>0:
    if os.path.isdir(args.exclude):
      ...
      for identity in seq_identity:
        ...
        embedding = get_embedding(args, _imgrec, identity, image_size, model, conf)
        #print(embedding.shape)
...
if __name__ == '__main__':
  ...
  # general
  parser.add_argument('--include', default='/mnt/cheolhui/Training_2/data/faces_emore,/mnt/cheolhui/Training_2/datai/faces_emore', type=str, help='')
  parser.add_argument('--exclude', default='', type=str, help='')
  parser.add_argument('--output', default='/mnt/cheolhui/Training_2/data/ms1m', type=str, help='')
  parser.add_argument('--model', default='r100_cosine', help='path to load model.')
  parser.add_argument('--batch-size', default=32, type=int, help='')
  parser.add_argument('--param1', default=0.65, type=float, help='')
